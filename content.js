...
(function() {
	...
	function injectCaptionInterceptor() {
		...
		script.src = browserAPI.runtime.getURL('youtube-api-reader.js');
	...
	function saveProgress() {
		...
		browserAPI.storage.local.get(['watchData'], (result) => {
			if (browserAPI.runtime.lastError) {
				console.log('Mikan: Storage error:', browserAPI.runtime.lastError);
				return;
			}
	...
	function finishSubmitPoints(current, gain, url) {
		let newTotal = current + gain;
		...
		notif.className = "mikan-point-notif"
		notif.innerText = "+" + gain;
		document.documentElement.appendChild(notif);
		window.setTimeout(() => { notif.remove(); }, 3999);
	}

	function handlePointAdd(amount) {
		console.log("Mikan: Attempting point increment");
		const xhr = new XMLHttpRequest();
		xhr.open("GET", pointsURL);
		xhr.send();
		xhr.responseType = "json"
		xhr.onload = () => {
			if (xhr.readyState == 4 && xhr.status == 200) {
				const data = xhr.response;
				finishSubmitPoints(data.points, amount, pointsURL);
			} else {
	...
	function handleTimeUpdate() {
		...
		if (lastRealTime > 0) {
			const realDelta = (now - lastRealTime) / 1000;  // Convert ms to seconds

			if (realDelta > 0 && realDelta < 2) {
				totalWatchedSeconds += realDelta;
				secondsUntilPoint += realDelta;
				if (secondsUntilPoint >= minuteInterval * 60 && gotPointData === true) {
					handlePointAdd(pointsReward);
					secondsUntilPoint = 0;
				}
			}
		}
		...
		lastRealTime = now;
		lastTime = video.currentTime;  // Keep this for seek detection
	...
	function handlePlay() {
		if (!video) return;
		lastTime = video.currentTime;
		lastRealTime = Date.now();  // Add this
		lastSaveTime = Date.now();
	...
	function handlePause() {
		saveProgress();
		lastRealTime = 0;  // Add this - reset so we don't count paused time
		console.log('Mikan: Video paused, progress saved');
	}

	function handleSeeked() {
		lastTime = video.currentTime;
		lastRealTime = Date.now();  // Add this - reset after seeking
	}
	...
	function init() {
		...
		browserAPI.storage.local.get(['pointsReward', 'minuteFrequency', 'pointsURL'], (result) => {
			...
			gotPointData = true;
			console.log("Mikan: Successfully got points data");
		})
	}
